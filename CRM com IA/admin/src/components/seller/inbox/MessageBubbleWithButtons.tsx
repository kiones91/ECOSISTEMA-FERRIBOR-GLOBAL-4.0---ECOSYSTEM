...
import { resolveMessageDeliveryStatus } from '@/lib/messageDeliveryStatus';
...
export function MessageBubbleWithButtons({
  ...
  const effectiveStatus = resolveMessageDeliveryStatus(metadata, status, {
    isOutbound: !isVisitor,
  });

  const StatusIcon = () => {
    if (isVisitor || !effectiveStatus) return null;
