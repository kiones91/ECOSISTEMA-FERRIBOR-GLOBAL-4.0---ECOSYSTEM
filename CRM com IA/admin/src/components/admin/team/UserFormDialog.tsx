...
const DEFAULT_TEMP_PASSWORD = '#mude123';
...
export function UserFormDialog({ member, open, onOpenChange }: UserFormDialogProps) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (isEdit && member) {
        ...
        // Create via edge function (uses default password #mude123)
        const { data, error } = await supabase.functions.invoke('create-team-member', {
          body: {
            ...
            app_origin: getPublicAppUrl(),
          },
        });
        if (error) throw error;
        if ((data as any)?.error) throw new Error((data as any).error);

        const result = data as { temp_password?: string; login_url?: string; email_sent?: boolean };
        toast.success(
          result?.email_sent
            ? 'Usuário criado e e-mail de acesso enviado!'
            : 'Usuário criado! (e-mail não enviado — compartilhe o acesso manualmente)'
        );
        setCreatedInfo({
          email: general.email,
          full_name: general.full_name,
          recovery_whatsapp: general.recovery_whatsapp,
          temp_password: result?.temp_password || DEFAULT_TEMP_PASSWORD,
          login_url: result?.login_url || `${getPublicAppUrl()}/login`,
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
            ...
            <div className="space-y-1.5">
              ...
              {!isEdit && (
                <p className="text-xs text-muted-foreground">
                  O novo usuário receberá um e-mail com a senha provisória <code className="font-mono">#mude123</code> e será obrigado a trocá-la no primeiro acesso.
                </p>
              )}
            </div>
      ...
      {/* Post-create info dialog with WhatsApp invite */}
      <Dialog
        open={!!createdInfo}
        onOpenChange={(o) => {
          if (!o) {
          ...
          {createdInfo && (
            <div className="space-y-4">
              <div className="rounded-lg bg-muted p-4 space-y-2 text-sm">
                <div><span className="text-muted-foreground">Email:</span> <strong>{createdInfo.email}</strong></div>
                <div className="flex items-center gap-2">
                  <span className="text-muted-foreground">Senha provisória:</span>
                  <code className="px-2 py-0.5 rounded bg-background font-mono">{createdInfo.temp_password}</code>
                  <Button
                    type="button"
                    variant="ghost"
                    size="sm"
                ...
                <div className="text-xs text-muted-foreground">
                  {createdInfo.email_sent
                    ? 'E-mail de acesso enviado para o novo usuário.'
                    : '⚠️ E-mail não enviado — compartilhe os dados manualmente.'}
                </div>
              </div>

              {(() => {
                const inviteMessage =
                  `Olá ${createdInfo.full_name}! Seu acesso foi criado.\n\n` +
                  `Link: ${createdInfo.login_url}\n` +
                  `Email: ${createdInfo.email}\n` +
                  `Senha provisória: ${createdInfo.temp_password}\n\n` +
                  `Você será obrigado a trocar a senha no primeiro acesso.`;
                const text = encodeURIComponent(inviteMessage);
                const rawPhone = (createdInfo.recovery_whatsapp || '').replace(/\D/g, '');
                // Use wa.me with phone when available; otherwise copy and open WhatsApp Web
                ...
                const handleOpen = async () => {
                  if (!rawPhone) {
                    try {
                      ...
                      toast.success('Mensagem copiada — cole na conversa do WhatsApp.');
                    } catch {
                      // ignore
                    }
                  }
                  // Use a real anchor click to bypass preview iframe popup blockers
                ...
                return (
                  <Button
                    type="button"
                    variant={rawPhone ? 'default' : 'outline'}
                    className="w-full"
                    onClick={handleOpen}
                  >
                    {rawPhone ? 'Enviar convite por WhatsApp' : 'Abrir WhatsApp para enviar convite'}
                  </Button>
                );
              })()}
