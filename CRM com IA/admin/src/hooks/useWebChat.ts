...
/** Atualiza cache da conversa aberta com mensagem inbound/outbound (realtime). */
export function mergeMessageIntoConversationCache(
  ...
  queryClient.setQueryData(['webchat-conversation', conversationId], (old: any) => {
    ...
    if (incoming._delivery_patch && incoming.id) {
      const idx = msgs.findIndex((m: any) => m.id === incoming.id);
      if (idx >= 0) {
        const next = msgs.slice();
        const { _delivery_patch: _, ...rest } = incoming;
        next[idx] = { ...msgs[idx], ...rest, metadata: rest.metadata ?? msgs[idx].metadata };
    ...
    const tempId = incoming.client_temp_id as string | undefined;
    if (tempId) {
...
export function useWebChatConversation(conversationId: string) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const token = (await getFreshAccessToken()) ?? session?.access_token;
      const url = `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/webchat-inbox?action=conversation&id=${conversationId}&msg_limit=100`;
      const res = await fetch(url, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json',
      ...
      return {
        ...
        hasMoreMessages: !!data.has_more_messages,
        oldestMessageAt: data.oldest_message_at as string | null,
      };
    },
    ...
    initialData: () => {
      ...
      const caches = queryClient.getQueriesData<any>({ queryKey: ['webchat-conversations'] });
      for (const [, data] of caches) {
        const items: any[] = Array.isArray(data) ? data : (data?.conversations || []);
        const found = items.find((c) => c?.id === conversationId);
        if (found) {
          return {
            conversation: found as WebChatConversation,
...
/** Carrega mensagens mais antigas (scroll infinito para cima). */
export function useLoadOlderMessages() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      const token = (await getFreshAccessToken()) ?? session?.access_token;
      const url = `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/webchat-inbox?action=conversation&id=${conversationId}&before=${encodeURIComponent(before)}&msg_limit=50`;
      const res = await fetch(url, {
        headers: {
          Authorization: `Bearer ${token}`,
          'Content-Type': 'application/json',
      ...
      if (!res.ok) {
        ...
        throw new Error(body?.error || 'Failed to load older messages');
      }
      return res.json();
    },
    onSuccess: (data, { conversationId }) => {
      queryClient.setQueryData(['webchat-conversation', conversationId], (old: any) => {
        ...
        const older = (data.messages || []).filter((m: any) => !existingIds.has(m.id));
        return {
          ...old,
          messages: [...older, ...(old.messages || [])],
          hasMoreMessages: !!data.has_more_messages,
          oldestMessageAt: older[0]?.created_at ?? data.oldest_message_at ?? old.oldestMessageAt,
        };
      });
    },
  });
...
export function useSendAgentMessage() {
  ...
  return useMutation({
    mutationFn: async ({ conversationId, content, replyToMessageId, media, clientTempId }: SendAgentMessageVars & { clientTempId?: string }) => {
      ...
      const metaErr = typeof data?.meta_error === 'string' ? data.meta_error : '';
      const deliveryFailed =
        data?.meta_delivery_ok === false ||
        data?.message?.metadata?.delivery_status === 'failed';
      if (!res.ok || deliveryFailed) {
        throw new Error(
          metaErr ||
            data?.error ||
            'Não foi possível entregar na Meta. Peça deploy de webchat-inbox build 2026.06.01.1 no Supabase.',
        );
      }
      return data;
    },
...
export function useSetConversationProduct() {
  ...
  return useMutation({
    ...
    onSuccess: (_, { conversationId }) => {
      ...
      queryClient.invalidateQueries({ queryKey: ['pipeline-stages-for-lead'] });
