...
export function useInvitationByToken(token: string | null) {
  return useQuery({
    ...
    queryFn: async () => {
      ...
      if (error) {
        console.error('[get_invitation_by_token]', error);
        return null;
      }
      ...
      return {
        ...
        organization: row.organization_id
          ? { id: row.organization_id, name: row.organization_name, logo_url: row.organization_logo_url }
          : null,
        squad: row.squad_id
          ? { id: row.squad_id, name: row.squad_name, color: row.squad_color }
          : null,
      };
    },
    enabled: !!token,
  ...
  return useMutation({
    mutationFn: async ({ email, role, squadId }: {
      ...
      try {
        ...
        await supabase.from('team_invitations').delete().eq('id', data.id);
        throw emailErr;
      }
