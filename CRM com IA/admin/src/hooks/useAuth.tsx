...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  // Single-tenant: a empresa é dona do CRM. Todo usuário autenticado tem acesso
  // total — sem distinção de papel. Mantemos as funções por compatibilidade de
  // chamadas existentes, mas todas liberam.
  const isAdmin = () => true;
  const isManager = () => true;
  const isSeller = () => true;
  const isSuperAdmin = () => true;

  const hasPermission = (_key: keyof UserPermissions) => true;
