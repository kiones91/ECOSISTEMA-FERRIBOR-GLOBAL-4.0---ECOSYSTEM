...
/** Consulta GET /instance/status na Evolution GO e espelha no banco. */
export async function syncEvolutionInstanceStatus(instanceId?: string) {
  const { data, error } = await supabase.functions.invoke('evolution-proxy', {
    body: { action: 'sync_instance_status', ...(instanceId ? { id: instanceId } : {}) },
  });
  if (error) throw error;
  return data;
}
