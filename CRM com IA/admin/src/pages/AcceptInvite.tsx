...
import { getPostInvitePath } from '@/lib/postLoginRoute';
...
export default function AcceptInvite() {
  ...
  const handleAcceptAsLoggedInUser = async () => {
    ...
    try {
      ...
      setTimeout(() => navigate(getPostInvitePath(invitation?.role)), 2000);
    } catch (error: any) {
  ...
  const handleCreateAccount = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (Array.isArray(identities) && identities.length === 0) {
        ...
        setTimeout(() => navigate(`/login?redirect=/aceitar-convite?token=${encodeURIComponent(token!)}`), 1500);
        return;
      }
