# 📋 BRIEFING TÉCNICO - PROJETO FERRIBOR GLOBAL 4.0 (REORGANIZADO)
...
### 3.1 Propósito e Requisitos Gerais
- **Propósito:** Captação de novos clientes, apresentação da FerriBor, mostruário de produtos e soluções, ponto de entrada para o Portal do Cliente.
- **Botão "Acessar seu Portal do Cliente":** Deve ser um destaque no site, levando os clientes cadastrados ou novos para o **Dashboard do Cliente** para login ou cadastro.

### 3.2 Site Institucional Multilíngue
...
### 3.3 FerriBor SmartSpec (IA de Análise de Desenhos - Frontend para Demonstração)

#### Propósito:
- Oferecer uma demonstração da capacidade de IA da FerriBor diretamente no site, atraindo leads qualificados.
- O upload e a análise inicial ocorrem no site, com a inteligência de backend integrada.

#### Arquitetura Técnica (Frontend Interação):
[Upload do Cliente no Site]
↓
[Integração Frontend com API de Backend (SmartSpec)] → Envio do arquivo para análise
↓
[Output no Site] → Recomendação de produto + Ficha técnica + Cotação estimada (interface para o cliente)

#### Requisitos Funcionais:
- Interface de upload de arquivos: PDF, DWG, STEP, imagens (JPG/PNG)
- Exibição de barra de progresso ("Analisando dimensões... Consultando catálogo...")
- Apresentação do Output: Recomendação de produto ideal + Ficha técnica + Cotação estimada

### 3.4 Calculadora de ROI e OEE (Ferramenta no Site)

#### Propósito:
- Prover uma ferramenta interativa no site para que potenciais clientes possam estimar benefícios financeiros e operacionais ao usar produtos FerriBor.
...
### 3.5 Biblioteca Técnica 3D (Mostruário de Peças no Site)

#### Propósito:
- Servir como um mostruário avançado de produtos no site, permitindo aos visitantes explorar as peças em detalhe.
...
#### Stack Recomendada (para o Site - mais detalhes na Seção 7):
...
## 💬 4. CRM COM IA (BACKEND/CORE)

### 4.1 Propósito e Requisitos Gerais
- **Propósito:** Gerenciar o relacionamento com clientes e fornecedores, centralizando comunicações, qualificando leads, roteando atendimentos e fornecendo histórico completo.
- **Integração Direta com Dashboard do Cliente:** Deve ser o *backbone* para todas as informações e interações exibidas no Dashboard do Cliente, incluindo a aba de mensagens diretas.
- **Histórico Completo:** O CRM interno deve ter acesso a todo o histórico do cliente, incluindo interações do site (cotações, análises SmartSpec), pedidos do Dashboard e comunicações diretas.

### 4.2 CRM Omnichannel com IA Poliglota
...
- Histórico completo traduzido e organizado (visível no CRM interno)
...
### 4.3 Formulário Inteligente de Cotação (Processamento Backend)

#### Propósito:
- Centralizar o recebimento e processamento de solicitações de cotação de forma automatizada e inteligente.

#### Requisitos Funcionais:
- Recebimento de Upload de arquivos: PDF, DWG, STEP, imagens (JPG/PNG)
- Extração automática de metadados (material, dimensões, volume) via IA/Middleware CAD
- Priorização automática da fila para engenharia
- Confirmação automática por WhatsApp/E-mail no idioma do cliente
- Status tracking da cotação (atualizado no CRM e visível no Dashboard do Cliente)

### 4.4 Dashboard de Leads em Tempo Real (Visão Interna do CRM)
...
#### Stack Recomendada (para o CRM - mais detalhes na Seção 7):
- **Backend Framework:** Node.js (NestJS) ou Python (FastAPI)
- **Banco de Dados Relacional:** PostgreSQL
- **IA e APIs:** OpenAI GPT-4o / Anthropic Claude 3.5 Sonnet (para NLU, detecção de idioma)
- **Integrações:** Sistema próprio para WhatsApp, SendGrid/AWS SES para E-mail, Middleware CAD (Aspose).


## 🖥️ 5. DASHBOARD DO CLIENTE (EXTRANET FRONTEND)

### 5.1 Propósito e Requisitos Gerais
- **Propósito:** Oferecer um portal exclusivo para clientes cadastrados, com acesso a informações personalizadas, histórico, ferramentas de gestão e comunicação direta com a FerriBor.
- **Interligado ao CRM de Atendimento:** Todas as informações do dashboard são alimentadas pelo CRM e as interações (como mensagens) são registradas lá.
- **Aba de Mensagens Diretas:** Um canal de comunicação direto com o time de engenharia/atendimento da FerriBor, com histórico visível para o cliente e o CRM.

### 5.2 FerriBor Extranet (Portal do Cliente)

#### Requisitos Funcionais:
- Login seguro (2FA recomendado)
- Dashboard personalizado por cliente
- Histórico completo de pedidos
- Status de produção em tempo real (Track & Trace Industrial - *integração com backend*):
  - "Em corte"
  - "Em vulcanização"
  - "Em controle de qualidade"
  - "Em expedição"
- Central de documentos (NFs, certificados, laudos técnicos)
- Chat direto com engenharia da FerriBor (funcionalidade de "Aba de Mensagens Diretas")

### 5.3 Recompra Automática

#### Propósito:
- Simplificar e automatizar o processo de recompra para os clientes, baseando-se no histórico e na vida útil dos produtos.

#### Requisitos Funcionais:
- Botão "Recomprar" para pedidos anteriores (1-click)
- Alertas preditivos de desgaste (Push notification PWA + WhatsApp + E-mail - *enviados via backend*):
  - Cruzamento de histórico de compras com vida útil do produto
  - Envio automático de notificações quando peça está chegando no fim da vida útil
- Assinatura de contratos de fornecimento recorrente

### 5.4 FerriBor Credit (Cashback B2B)

#### Propósito:
- Incentivar a fidelidade e engajamento dos clientes através de um sistema de pontos e recompensas.
...
### 5.5 FerriBor Circular (Logística Reversa - Iniciada pelo Cliente)

#### Propósito:
- Facilitar o processo de logística reversa para peças usadas, promovendo a sustentabilidade e gerando créditos ESG para os clientes.
...
- Ordem de coleta automática (*processado no backend*)
- Rastreamento da coleta

### 5.6 Dashboard e Selo ESG (Visível ao Cliente)

#### Propósito:
- Fornecer aos clientes evidências do seu impacto positivo no ESG através de certificações e um dashboard de métricas.
...
### 5.7 Portal de Fornecedores (Integração Backend - Visto no Dashboard Administrativo Interno)

#### Propósito:
- Facilitar a integração com ERPs de grandes mineradoras para otimizar processos de pedidos e compliance.
...
#### Stack Recomendada (para o Dashboard do Cliente - mais detalhes na Seção 7):
- **Frontend Framework:** Next.js (React) + TypeScript
- **PWA Support:** Next.js com PWA plugin, Service Workers, IndexedDB, Web Push API, Manifest.json
...
| :--- | :--- |
| Framework | Next.js (React) / Nuxt.js |
| UI Framework | Tailwind CSS + shadcn/ui |
...
| Componente | Duração | Entregas Principais |
| :--- | :--- | :--- |
| **SITE** | Mês 1-3 | Site trilíngue, SmartSpec (Frontend), Calculadora ROI, Mostruário 3D, SEO técnico |
| **CRM COM IA** | Mês 1-6 | CRM Poliglota, Formulário de Cotação (backend), Dashboard de Leads |
| **DASHBOARD DO CLIENTE** | Mês 4-9 | Extranet, Recompra Automática (frontend), FerriBor Credit, FerriBor Circular (frontend), Dashboard e Selo ESG (frontend) |
| **INTEGRAÇÕES AVANÇADAS** | Mês 10-12 | Portal de Fornecedores (API/EDI), Backends de Recompra, ESG, Créditos |
...
3. **Performance mobile-first** (PWA e Dashboard do Cliente precisam ser rápidos)
...
- O PWA e o Dashboard do Cliente precisam funcionar bem offline (minas sem internet)
- Integração com WhatsApp via QR Code (sistema próprio da startup)
- A integração entre CRM e Dashboard do Cliente é crucial para a experiência do usuário e a visão interna.
