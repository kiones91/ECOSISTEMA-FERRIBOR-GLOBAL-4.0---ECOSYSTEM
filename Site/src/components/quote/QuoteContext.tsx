...
const QuoteContext = createContext<QuoteContextType>({ openQuoteModal: () => {} });
    ...
    <QuoteContext.Provider value={{ openQuoteModal }}>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm transition-all duration-300">
          <div className="relative w-full max-w-lg bg-[#F8FAF9] rounded-3xl border border-white/50 p-8 shadow-2xl flex flex-col">
            <button onClick={() => setIsModalOpen(false)} className="absolute top-4 right-4 text-slate-400 hover:text-slate-900 cursor-hover">
            ...
            <form className="space-y-5" onSubmit={(e) => { e.preventDefault(); setIsModalOpen(false); setIsSuccessModalOpen(true); }}>
              ...
              <div className="flex flex-col gap-1">
                ...
                <select id="input-modal-service" value={selectedService} onChange={(e) => setSelectedService(e.target.value)} className="w-full bg-transparent border-b border-slate-300 py-2.5 text-xs text-slate-800 focus:border-red-600 outline-none">
              ...
              <div className="input-group">
                <textarea className="w-full bg-transparent border-none border-b border-slate-300 outline-none p-2 text-xs focus:border-red-600 transition-colors" placeholder="Especificações do elastômero (dureza, composto, dimensões)" required rows={2}></textarea>
              </div>
              ...
              <div className="flex gap-3 pt-2">
                ...
                <button type="button" onClick={() => setIsModalOpen(false)} className="btn-community cursor-hover">
                  <span>Cancelar</span>
                </button>
              </div>
            </form>
      ...
      {isSuccessModalOpen && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
          <div className="relative w-full max-w-sm bg-white rounded-3xl p-8 border border-white/50 shadow-2xl text-center flex flex-col items-center">
            ...
            <p className="text-slate-500 text-xs leading-relaxed mb-6">Sua solicitação foi processada com sucesso. Nossa equipe comercial/técnica entrará em contato em breve.</p>
            <button onClick={() => setIsSuccessModalOpen(false)} className="px-8 py-2.5 bg-slate-900 text-white rounded-xl text-xs font-bold uppercase tracking-widest hover:bg-green-600 hover:shadow-md transition-all duration-300 cursor-hover">
              Fechar
            </button>
          </div>
        </div>
